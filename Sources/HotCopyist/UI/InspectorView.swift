...
struct InspectorView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 3) {
            HStack(spacing: 8) {
                ...
                headerButton("Copy as Text") { copyDump() }
                headerButton("Export…") { exportRaw() }
    ...
    private var metaString: String {
        ...
        parts.append(Fmt.full.string(from: item.copiedAt))
                    ...
                    Button {
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 6, style: .continuous)
                                .fill(selected ? Theme.mint.opacity(0.12) : Color.primary.opacity(0.05))
                        )
                        .overlay(
                            RoundedRectangle(cornerRadius: 6, style: .continuous)
            ...
            ForEach(DecodeMode.allCases, id: \.self) { candidate in
                ...
                Button {
                    ...
                    Text(candidate.rawValue)
                        ...
                        .background(
                            Capsule().fill(selected ? Theme.mint : Color.primary.opacity(0.05))
                        )
                }
                .buttonStyle(.plain)
            }
    ...
    /// Piano-roll rendering of a Logic clip, shown above the raw payload.
    private func musicSection(_ summary: MidiClipSummary) -> some View {
    ...
    private func musicMetaString(_ summary: MidiClipSummary) -> String {
        var parts: [String] = []
        let bars = summary.barCount
        if bars > 0 { parts.append(bars == 1 ? "1 bar" : "\(bars) bars") }
        parts.append("\(summary.noteCount) \(summary.noteCount == 1 ? "note" : "notes")")
        if summary.trackCount > 1 { parts.append("\(summary.trackCount) tracks") }
        if let range = summary.pitchRange {
            parts.append(range.count == 1
                ? MidiClipSummary.noteName(range.lowerBound)
                : "\(MidiClipSummary.noteName(range.lowerBound))–\(MidiClipSummary.noteName(range.upperBound))")
        }
        parts.append("\(summary.timeSigNumerator)/\(summary.timeSigDenominator)")
        return parts.joined(separator: " · ")
    }
    ...
    private func copyDump() {
        guard let rep = currentRep else { return }
        var dump = "HotCopyist payload dump\n"
        dump += "type: \(rep.type)\n"
...
enum PayloadDecoder {
    ...
    static func auto(_ data: Data, type: String) -> Payload {
        ...
        if let string = String(data: data, encoding: .utf8), isMostlyPrintable(string) {
            // Logic's event-list mirrors get a plain-English translation
            // followed by the raw rows (Text mode shows them untouched).
            if let rows = LogicEventText.parse(string) {
                return .text(LogicEventText.translated(rows) + "\n\n— original —\n\n" + string)
            }
            return .text(string)
        }
    ...
    static func hexDump(_ data: Data, limit: Int = 16_384) -> String {
        ...
        if data.count > limit {
            lines.append("")
            lines.append("… truncated — showing \(Fmt.size(limit)) of \(Fmt.size(data.count)). Use Export for the full payload.")
        }
